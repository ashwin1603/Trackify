...
import { LayoutDashboard, ClipboardEdit, Bug } from "lucide-react";

const navItems = [
  { to: "/developer", end: true, icon: LayoutDashboard, label: "Dashboard" },
  { to: "/developer/work-status", icon: ClipboardEdit, label: "Work Status Panel" },
];
    ...
    <aside className="flex h-screen w-64 flex-shrink-0 flex-col bg-[#0b1120] border-r border-slate-800">
      ...
      <div className="flex items-center gap-3 px-6 py-6 border-b border-slate-800">
        ...
        <div>
          ...
          <p className="text-xs text-slate-400 leading-tight">Developer Panel</p>
        </div>
      </div>
      ...
      <div className="px-6 py-4 border-t border-slate-800">
        <p className="text-xs text-slate-600">v1.0.0 — Developer Build</p>
      </div>
    </aside>
  );
}
